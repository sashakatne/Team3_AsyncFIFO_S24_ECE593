#!/usr/bin/env python3
"""Render a focused flag debug waveform from the farm flag_threshold.vcd.

Tracks included by request:
  wclk, rclk, write/read pointer values in decimal, wen, ren, and all four flags.

Clock rising edges are highlighted with red ticks on the clock tracks.
"""
...
def window_rows(rows: list[dict[str, str]], start_ns: float, end_ns: float) -> list[dict[str, str]]:
    if start_ns <= 0 and (not rows or end_ns >= float(rows[-1]["time_ns"])):
        return rows
...
def render_svg(
    ...
    start_ns: float,
    end_ns: float,
    rising_edges: dict[str, list[float]],
    svg_out: Path,
    png_out: Path,
    title: str,
) -> None:
    if not rows:
        raise SystemExit("no debug waveform rows parsed")
    ...
    parts.append(f'<rect width="{width}" height="{height}" fill="#fbfbfb"/>')
    parts.append(
    ...
    for idx, key in enumerate(keys):
        ...
        if key in rising_edges:
            for edge_t in rising_edges[key]:
                if edge_t < start_ns or edge_t > end_ns:
                    continue
                x = x_of(edge_t)
                parts.append(
                    f'<line x1="{x:.1f}" y1="{y0 + 2}" x2="{x:.1f}" y2="{y1 - 2}" '
                    f'stroke="#d72638" stroke-width="1.2" opacity="0.65"/>'
                )
        ...
        for t0, t1, value in segments:
            ...
            if kind == "digital":
                ...
                color = "#006600" if value == "1" else "#333333"
                parts.append(f'<line x1="{x0:.1f}" y1="{y:.1f}" x2="{x1:.1f}" y2="{y:.1f}" stroke="{color}" stroke-width="2"/>')
            else:
                parts.append(
                    f'<rect x="{x0:.1f}" y="{y0 + 7:.1f}" width="{max(x1 - x0, 1):.1f}" '
                    f'height="{track_h - 14:.1f}" fill="#eef3ff" stroke="#0066cc" stroke-width="0.8"/>'
                )
                if x1 - x0 > 22:
                    parts.append(f'<text x="{(x0 + x1) / 2:.1f}" y="{mid + 4:.1f}" font-size="10" text-anchor="middle" fill="#114">{value}</text>')

        if kind == "digital":
            for seg_idx, (t0, t1, value) in enumerate(segments[:-1]):
                next_value = segments[seg_idx + 1][2]
                if next_value == value:
                    continue
                x = x_of(t1)
                y_from = y0 + 9 if value == "1" else y1 - 9
                y_to = y0 + 9 if next_value == "1" else y1 - 9
                parts.append(f'<line x1="{x:.1f}" y1="{y_from:.1f}" x2="{x:.1f}" y2="{y_to:.1f}" stroke="#777" stroke-width="1"/>')

    parts.append("</svg>")
    svg_out.write_text("\n".join(parts) + "\n")
    convert_png(svg_out, png_out)


def convert_png(svg_out: Path, png_out: Path) -> None:
    if shutil.which("sips") is None:
        print("sips not found; PNG conversion skipped")
        return
...
def main() -> None:
    ...
    render_svg(
        ...
        SVG_OUT,
        PNG_OUT,
        f"M3 directed flag debug waveform from farm VCD, 0 - {t_max:.0f} ns",
    )
    render_svg(
        window_rows(rows, 560, 1085),
        560,
        1085,
        rising_edges,
        WRITE_ZOOM_SVG,
        WRITE_ZOOM_PNG,
        "M3 write-side threshold zoom: half-full and full",
    )
    render_svg(
        window_rows(rows, 1720, 2470),
        1720,
        2470,
        rising_edges,
        READ_ZOOM_SVG,
        READ_ZOOM_PNG,
        "M3 read-side threshold zoom: half-empty and empty",
    )
    print(
        "wrote "
        f"{CSV_OUT.name}, {SVG_OUT.name}, {PNG_OUT.name}, "
        f"{WRITE_ZOOM_SVG.name}, {WRITE_ZOOM_PNG.name}, "
        f"{READ_ZOOM_SVG.name}, {READ_ZOOM_PNG.name}"
    )
