#!/usr/bin/env python3
"""Render readable flag-threshold waveforms from flag_threshold.vcd."""
...
TRACKS = [
    ...
    ("wptr_dec", "bus", ["flag_threshold_tb"], "wptr", "wptr dec"),
    ("rptr_dec", "bus", ["flag_threshold_tb"], "rptr", "rptr dec"),
    ("wen", "digital", ["flag_threshold_tb"], "wen", "wen"),
    ("ren", "digital", ["flag_threshold_tb"], "ren", "ren"),
    ("full", "digital", ["flag_threshold_tb"], "full", "full"),
    ("half_full", "digital", ["flag_threshold_tb"], "half_full", "half_full"),
    ("empty", "digital", ["flag_threshold_tb"], "empty", "empty"),
    ("half_empty", "digital", ["flag_threshold_tb"], "half_empty", "half_empty"),
]
...
def render_svg(
    ...
    parts = [
        ...
        "Red ticks mark rising clock edges. Pointer tracks show binary pointer locations in decimal.</text>",
    ]
...
def main() -> None:
    ...
    write_csv(rows)
    render_svg(window_rows(rows, 0, t_max), 0, t_max, rising_edges, SVG_OUT, PNG_OUT, f"Miscellaneous directed flag debug waveform, 0 - {t_max:.0f} ns")
    render_svg(window_rows(rows, 560, 1110), 560, 1110, rising_edges, WRITE_ZOOM_SVG, WRITE_ZOOM_PNG, "Miscellaneous write-side threshold zoom: half-full and full")
    render_svg(window_rows(rows, 1780, 2520), 1780, 2520, rising_edges, READ_ZOOM_SVG, READ_ZOOM_PNG, "Miscellaneous read-side threshold zoom: half-empty and empty")
    print(
