...
def main() -> None:
    ...
    write_csv(rows)
    render_svg(window_rows(rows, 0, t_max), 0, t_max, rising_edges, SVG_OUT, PNG_OUT, f"M4 directed flag debug waveform from farm VCD, 0 - {t_max:.0f} ns")
    render_svg(window_rows(rows, 560, 1085), 560, 1085, rising_edges, WRITE_ZOOM_SVG, WRITE_ZOOM_PNG, "M4 write-side threshold zoom: half-full and full")
    render_svg(window_rows(rows, 1720, 2470), 1720, 2470, rising_edges, READ_ZOOM_SVG, READ_ZOOM_PNG, "M4 read-side threshold zoom: half-empty and empty")
    print(
